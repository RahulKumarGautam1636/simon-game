...
// import { BASE_URL, TAKE_HOME_ID, XYZ_ID } from '../../../constants';
...
//             if (isRestaurant) {
//                 if (res.data === 'N') {return alert('Failed to Place Order.');};
...
//         Rate: i.Rate,
//         PackSizeId: i.PackSizeId ? i.PackSizeId : 0,
//         Amount: i.Delstatus ? 0 : num(i.Amount),
...
//     const submitNote = (item, orderType, modifyIndex) => {
...
//     const handleDeletePreviousOrderItem = (deleteIndex, flag) =>  {
//         setPreviousOrder(pre => {
//             let list = [ ...pre.data.SalesObj.SalesDetails ];   
//             if (deleteIndex === undefined) return;
//             list[deleteIndex].Delstatus = flag;               
//             let newState = {...pre, data: { SalesObj: { ...pre.data.SalesObj, SalesDetails: list } }};              
//             return newState;
//         });
//     }
...
//                                             <Fragment key={index}>
//                                                 <tr className={`cart_item bg-white ${item.Delstatus && 'text-decoration-line-through'}`}>
//                                                     <td className="cart-product-name"> 
...
//                                                     <td className="cart-product-total text-end text-nowrap">
//                                                         <span className="amount">₹ {item.Amount}</span> 
//                                                         {item.Delstatus ? 
//                                                             <i onClick={() => handleDeletePreviousOrderItem(index, false)} className='bx bxs-share ms-md-2 text-purple-600 fw-bold' role='button' style={{fontSize: '1.3em', verticalAlign: 'sub'}}></i>
//                                                         :
//                                                             <i onClick={() => handleDeletePreviousOrderItem(index, true)} className='bx bx-x-circle ms-md-2 text-danger circle-rounded' role='button' style={{fontSize: '1.3em', verticalAlign: 'sub'}}></i>
//                                                         }
//                                                     </td> 
//                                                 </tr>
//                                                 {item.Specification ? <tr>
//                                                     <td className={`cart_item p-2 ${item.Delstatus && 'opacity-25'}`} colSpan={4} style={{background: 'aliceblue'}}>
//                                                         <i className='bx bx-subdirectory-right me-2' style={{transform: 'scale(1.6)'}}></i> {item.Specification} <i className='bx bx-message-square-x ms-2' onClick={() => {setNote({id: index, text: ''}); submitNote(item, 'PREVIOUS_ORDER', index)}} role='button' style={{transform: 'scale(1.2)'}}></i>
//                                                     </td>
//                                                 </tr> : ''}
//                                                 {note.id === index ? <tr>
//                                                     <td className="cart-product-total" colSpan={4}>
//                                                         <form className="input-group flex-nowrap position-relative" onSubmit={() => submitNote(item, 'PREVIOUS_ORDER')}>
//                                                             <input className="form-control fw-bold" onChange={(e) => setNote(pre => ({...pre, text: e.target.value}))} value={note.text} name="Note" type="text" placeholder="Write Notes" />
//                                                             <button type='submit' className="input-group-text">Add Note</button>
//                                                             <button type='button' onClick={() => setNote({id: '', text: ''})} className="input-group-text">
//                                                                 <i className="bx bx-x-circle text-danger circle-rounded text-7 lh-1" role="button"></i>
//                                                             </button>
//                                                         </form>
//                                                     </td>
//                                                 </tr>: ''}
