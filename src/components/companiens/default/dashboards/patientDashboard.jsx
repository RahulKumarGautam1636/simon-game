...
const PatientDashboard = ({ isLoggedIn, compCode, userInfo, loaderAction, compInfo, userInfoAction, isMobile, bookingInfo, globalData }) => {
  ...
  //   if (bookingInfo.AppointDate) {
  //     const [dd, mm, yyyy] = bookingInfo.AppointDate.split('/');
  //     let x = `${mm}/${dd}/${yyyy}`;
  //     let appnDate = new Date(x).getDate();
  //     const currDate = new Date().getDate();
  //     if (appnDate > currDate) {
  //       getTabData("UENQ");  
  //       setActiveDayTab('upcoming_appointments')      
  //     } else if (appnDate < currDate) {
  //       getTabData("PENQ");        
  //       setActiveDayTab('previous_appointments')      
  //     } else {
  //       getTabData("ENQ");
  ...
  //     getTabData("ENQ");
  //   }
  // },[userInfo.PartyCode, userInfo.selectedCompany.EncCompanyId])
  ...
  const renderAppnData = (data, viewType) => {
    if (data.loading) {
      ...
      return data.data.PartyFollowupList.map((item, index) => {
          if (appnStyle === 'grid') return <DashboardAppnListCard key={item.AutoId} data={item} styles={{fontSize: '0.93em', maxWidth: '28.7em'}} />;
                              ...
                              <div className="tab-content">
                                <div className='tab-pane fade show active'>
                                  <div className="card card-table mb-0 border-0 px-1">
                                    <div className="card-body px-lg-0">
